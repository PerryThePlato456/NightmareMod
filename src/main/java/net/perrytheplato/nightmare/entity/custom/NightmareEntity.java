...
public class NightmareEntity extends HostileEntity {
    ...
    public static DefaultAttributeContainer.Builder createMobAttributes() {
        return MobEntity.createMobAttributes()
                .add(EntityAttributes.MAX_HEALTH, 70.0)
    ...
    public boolean teleportTo(double x, double y, double z) {
        ...
        if (this.getWorld().isSpaceEmpty(this, this.getBoundingBox().offset(pos.subtract(this.getBlockPos())))) {
    ...
    public void tick() {
        ...
        if (!this.getWorld().isClient()) {
            PlayerEntity target = this.getWorld().getClosestPlayer(this, -1);
            if (target != null && this.age % 200 == 0) {
                double offsetX = (this.random.nextDouble() - 0.5) * 16;
                double offsetZ = (this.random.nextDouble() - 0.5) * 16;
